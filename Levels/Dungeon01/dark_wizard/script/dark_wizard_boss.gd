...
func shoot_orb() -> void:
	...
	eb.global_position = boss_node.global_position + Vector2( 0, -5 )
...
func _trigger_enemy_counter() -> void:
	var ec = get_node_or_null("../EnemyCounter")
	if ec == null:
		ec = get_parent()
	if ec and ec is EnemyCounter:
		ec.enemies_defeated.emit()
